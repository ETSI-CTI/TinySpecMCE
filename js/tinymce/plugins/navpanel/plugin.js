...
const isHeaderNode = function(el){
	return ['h1','h2','h3','h4','h5','h6'].indexOf(el.nodeName.toLowerCase()) >= 0;
}

const subHeaderNode = function(el){
	for(var e=el.lastChild; e; e=e.previousSibling){
		var s = subHeaderNode(e);
		if(s) return s;
		if(isHeaderNode(e)) return e;
	}
	return null;
}
const previousHeaderNode = function(el){
	...
	if(el.parentNode && el.parentNode.nodeName.toLowerCase() != 'body'){
		if(isHeaderNode(el.parentNode)) return el.parentNode;
		e = previousHeaderNode(el.parentNode);
	}
	return null;
}

const NavTree = function(tinymce, editor) {
	...
	const forEachTocItem = function(cb, data){
		if(toc){
			for(var i=toc.firstChild; i; i=i.nextSibling) {
				if(i.nodeType == Node.ELEMENT_NODE){
					if(cb(i, data)) return i;
				}
			}
		}
		return null;
	...
	const tocItemById = function(id) {
		return forEachTocItem(function(el){
			return el.id == id ? el : null;
		});
	}

	const tocItemByElement = this.tocItemByElement = function(el){
		if(typeof(el) == 'string'){
			// search by id
	...
	const elementByTocItem = this.elementByTocItem = function(ti){
	...
	const selectTocItem = this.selectTocItem = function(tocItem) {
		if(selectedTocItem)selectedTocItem.classList.remove('current');
		tocItem.classList.add('current');
		selectedTocItem = tocItem;
	}

	const _on_item_click = function(ev){
	...
	const createTocItem = function(el){
		var tocItem;
		if(!el.id || !el.id.startsWith('hdr-')){
		...
		if(!tocItem){
			...
			tocItem.level = parseInt(el.nodeName.substr(1));
	...
	this.setContent = function(dom) {
		var i, l;
		var headers = editor.dom.select('h1,h2,h3,h4,h5,h6');
	...
	this.insertTocItemForElement = function(el) {
		...
		if(!tocItem.parentNode){
			...
			if(h)h = tocItemByElement(h);
			if(h)h = h.nextSibling;
			else h = toc.firstChild;
			if(h) tocItem = toc.insertBefore(tocItem, h);
			else  tocItem = toc.appendChild(tocItem);
		}
	}
	...
	this.removeTocItem = function(tocItem) {
		var el = elementByTocItem(tocItem);
		if(el){
			if(el.id && el.id.startsWith('hdr-'))el.id = undefined;
		}
		toc.removeChild(tocItem);
	}
	...
	var updateElements = {};
	var performUpdate = function(self){
		updateTimerId = false;
		for(var id in updateElements){
			var el = editor.dom.get(id);
			var ti;
			if(el){
				ti = self.tocItemByElement(el);
				if(ti){
					self.updateTocItem(ti, el);
	...
	editor.addCommand('mceShowNavPanel', function () {
		if(navPanel){
			...
			navPanel = tinymce.ui.Factory.create(
				{type: 'control', name: 'nav-panel', classes: 'nav-panel', title: 'Navigation'}
			);
